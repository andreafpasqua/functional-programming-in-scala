...
case class Population[+T](population: Stream[T], size: Long) {
...
object Sampler {
  ...
  def intInInterval(lb: Int, ub: Int): Sampler[Int] =
    Sampler(
      ...
      Some(Population(Stream.unfold(lb)(n => if (n < ub) Some((n, n + 1)) else None), (ub - lb).toLong))
    ).map(_ + lb)
  ...
  private def getPopulationForList[T](population: Population[T], n: Int): Population[List[T]] = {
    ...
    Population(go(Stream(List.empty[T]), n), math.pow(population.size, n).toLong)
  }
