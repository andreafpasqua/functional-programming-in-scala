...
trait Parsers[Parser[+_]] {
  ...
  def many[T](p: Parser[T]): Parser[List[T]] =
    or[List[T]](map2(p, many(p))(_ :: _), succeed[List[T]](Nil))
  ...
  def listOfN[T](p: Parser[T])(n: Int): Parser[List[T]] =
    if (n > 0) map2(p, listOfN(p)(n - 1))(_ :: _) else succeed[List[T]](Nil)
  ...
  /**
    ...
    * character c
    */
  def numFollowedByAsMany(c: Char): Parser[Int] = flatMap[String, Int](regex("[0-9]".r)){
    digit =>
      val _ = listOfN[Char](char(c))(digit.toInt)
      succeed[Int](digit.toInt)
  }
//    for {
//      digit <- regex("[0-9]".r)
...
//  implicit def operators[T](p: Parser[T]): ParserOps[T] = ParserOps[T](p)
  ...
  implicit case class ParserOps[T](p: Parser[T]) {
    ...
    def or[UU >: T](other: => Parser[UU]) = self.or(p, other)

    def |[UU >: T](other: => Parser[UU]) = self.or(p, other)

    def map[S](f: T => S): Parser[S] = self.map[T, S](p)(f)

    def **[S](other: => Parser[S]): Parser[(T, S)] = self.product[T, S](p, other)

    /**
      * It uses this to delete the corresponding portion of the input and
      * then parses what is left with other
      */
    def >>[S](other: Parser[S]): Parser[S] = self.map2[T, S, S](p, other){ case (t, s) => s}

    /**
      * It uses this to parse and other to delete its corresponding portion of the input
      * from what is left after this
      */
    def <<[S](other: Parser[S]): Parser[T] = self.map2[T, S, T](p, other){ case (t, s) => t}
