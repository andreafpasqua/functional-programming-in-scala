...
import andrea.scala.functional.programming.parsing2.MyParser.string
...
object JSONParser {
  ...
  val myParser = string("ciao").map(x => x)
  ...
  def jNumber: MyParser[JSON] = many("[0-9].".r).map(d => JNumber(d.reduce(_ + _).toDouble))
