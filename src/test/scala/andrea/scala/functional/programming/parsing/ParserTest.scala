...
//      */
//    def stringLaw: Prop =
//      Sampler.string(100).forall(s => string(s).run(s).result.right.get == s) &&
//        Sampler.listOfN(Sampler.string(100), 2).forall {
//          case List(s1, s2) if s1 != s2 => string(s1).run(s2).result.isLeft
//          case _ => true
//        }
//
//    /**
...
//        case (s1, s2) => p.label(s2).run(s1).result match {
//          case Left(ParserError((_, msg) :: list)) => msg == s2
//          case _ => true
//        }
//      }
//    }
