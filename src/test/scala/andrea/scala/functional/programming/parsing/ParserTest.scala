...
class ParserTest extends FunSuite {
  ...
  test("* Test Location line, column, parsed and unparsed") {
  ...
  test("* Test delay") {
    lazy val thunk = {
      println("\tParser evaluated"); succeed(10)
    }
  ...
  test("* Test countChar, countChar1, countTwoChars") {
  ...
  test("* Test succeedLaw") {
    assert(ParserLaws.succeedLaw(Sampler.intInInterval(-100, 101)).check(maxSize, numSamples).notFalsified)
  }

  test("* Test productLaw") {
    val sampler = Sampler.string(100)
    val functionSampler = Sampler.function(sampler, sampler)
    assert(ParserLaws.productLaw(parser, parser, parser)(functionSampler, functionSampler)
      .check(maxSize, numSamples).notFalsified)
  }

  test("* Test labelLaw") {
    assert(ParserLaws.labelLaw(parser)(Sampler.string(100)).check(maxSize, numSamples).notFalsified)
  }
