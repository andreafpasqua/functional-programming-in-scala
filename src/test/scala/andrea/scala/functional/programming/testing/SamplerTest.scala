...
class SamplerTest extends FunSuite {
  ...
  test("* Test isFalsified, notFalsified") {
  ...
  test("* Test forall for UnSizedSampler") {
  ...
  test("* Test max properties") {
    val variousLengths = BySizeSampler(n => double.listOf(unit(n)))
    def maxPredicate1(list: List[Double]) =
  ...
  test("* Test takeWhile and dropWhile properties") {
    /**
