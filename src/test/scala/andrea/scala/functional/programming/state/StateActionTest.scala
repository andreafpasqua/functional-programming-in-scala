...
object StateActionTest extends App {
  ...
  assert(
    StateAction.sequence(List(testAction1, testAction2, testAction2, testAction1, testAction2)).runAndGetValue(ts) ==
    List("1+", "2+", "4+", "5+", "10+"))
  assert(
    StateAction.sequence(List(testAction2, testAction1, testAction2, testAction2, testAction1)).runAndGetValue(ts) ==
      List("0+", "1+", "2+", "4+", "5+"))
